<?php require_once 'templates/mostrar/header.phtml'; ?>

<div class="container mt-1">
    <h5>Carga de película</h5>
    <form action="agregar_pelicula" method="POST">
        <div class="mb-1">
            <label for="actor" class="form-label">Actor</label>
            <select class="form-select" id="actor" name="id_actor" required>
                <?php foreach($actor as $act): ?>
                    <option value="<?= $act->id_actor; ?>"><?= $act->actor; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-1">
            <label for="nombre_pelicula" class="form-label">Nombre de la Película</label>
            <input type="text" class="form-control" id="nombre_pelicula" name="nombre_pelicula" required>
        </div>
        <div class="mb-1">
            <label for="duracion" class="form-label">Duración</label>
            <input type="number" class="form-control" id="duracion" name="duracion" required>
        </div>
        <div class="mb-1">
            <label for="genero" class="form-label">Género</label>
            <input type="text" class="form-control" id="genero" name="genero" required>
        </div>
        <div class="mb-1">
            <label for="descripcion" class="form-label">Descripción</label>
            <textarea class="form-control" name="descripcion" required></textarea>
        </div>
        <div class="mb-1">
            <label for="fecha_estreno" class="form-label">Fecha de estreno</label>
            <input type="date" class="form-control" id="fecha_estreno" name="fecha_estreno" required>
        </div>
        <div class="mb-1">
            <label for="publico" class="form-label">Público</label>
            <input type="text" class="form-control" id="publico" name="publico" required>
        </div>
        <div class="mb-1">
            <label for="img" class="form-label">URL Imagen</label>
            <input type="text" class="form-control" id="img" name="img" required>
        </div>
        <button type="submit" class="btn btn-primary btn-sm">Guardar</button>
    </form>
</div>

<?php require_once 'templates/mostrar/footer.phtml'; ?>
