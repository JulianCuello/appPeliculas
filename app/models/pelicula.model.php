...
class listamodel extends model{
    ...
    public function obtener_pelicula_con_actores($id){
        $query = $this->db->prepare(
            'SELECT pelicula.*, actor.nombre_actor, actor.id_actor 
             FROM pelicula 
    ...
    public function obtener_pelicula_actor_por_id($id){
        $query = $this->db->prepare(
            'SELECT pelicula.*, actor.nombre_actor 
             FROM actor 
    ...
    //insertar película (sin id_actor porque no existe en la tabla)
    ...
    //modifica producto (sin id_actor)
